...
#Logistic regression (~ indicates the use of all explanatory variables to predict the response variable)
classifier = glm(formula = died90 ~ .,
                 family = binomial,
                 data = training)

# Predicting the Test set results based on the explanatory variables
prob_pred = predict(classifier, type = 'response', newdata = testing[-18])
